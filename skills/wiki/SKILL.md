---
name: wiki
description: |
  Maintain the OpenClaw memory wiki vault — sources, concepts, entities, syntheses.
  Zero-bloat, zero-duplication. Sources are canonical. GitHub Pages renders every page.
  Always reply with direct link to deployed page after push.
triggers:
  - "maintain the wiki"
  - "add to the wiki"
  - "wiki entry"
  - "ingest into wiki"
  - "wiki source"
  - "wiki concept"
  - "capture this knowledge"
  - "remember this"
---

Use this skill when working inside the OpenClaw memory wiki vault at `~/.openclaw/wiki/main/`.

## ACTUAL Vault Architecture

**This is the canonical structure. Do not follow any other description.**

```
wiki vault/                              ← ~/.openclaw/wiki/main/
├── sources/        ← CANONICAL CONTENT (this is where content lives)
│   ├── NNNN-*.md   ← raw content, full text, with frontmatter
│   └── index.md
├── concepts/       ← CLAIMS + RELATIONSHIPS only (metadata, links to sources/)
│   ├── *.md
│   └── index.md
├── entities/       ← People and organizations
│   ├── *.md
│   └── index.md
├── syntheses/      ← Auto-generated reports
├── reports/        ← Auto-generated diagnostics (lint, stale, provenance)
├── index.md        ← Master catalog
└── manifest.json   ← Machine-readable index (regenerated after each operation)
```

**Critical rule: `sources/` is canonical. One document lives in one file in `sources/`.**
- `concepts/` pages have frontmatter + claims + wikilinks to `sources/` — they do NOT duplicate content
- `entities/` pages have frontmatter + description — they do NOT duplicate content
- GitHub Pages renders every file from `sources/` directly (no code fences)

## Four-Output Pipeline

All agent outputs go to public repos. This skill manages the **wiki** output only.

| Output | Repo | Skill | URL pattern |
|--------|------|-------|-------------|
| Wiki | `genesis-zero-bot-wiki` | `wiki` | `sources/NNNN-slug.html` |
| ADRs | `genesis-zero-bot-adrs` | `adrs` | `adr-NNNN.html` |
| Specs | `genesis-zero-bot-spec` | `spec` | `feature-*-slug.html` |
| Archi | `genesis-zero-bot-arch` | `arch` | `html/index.html` |

## Adding a New Document — Full Workflow

### Step 1 — Determine the next number

```bash
cd ~/.openclaw/wiki/main
ls sources/ | grep -E '^[0-9]+-' | sort -t'-' -k1 -n | tail -1
# Output looks like: 0008-topic-57-airic-easm-extracted.md
# Next number is 0009
```

### Step 2 — Create source file

Write the source file to `sources/NNNN-title-slug.md`. Replace NNNN with the next sequential number.

**SOURCE TEMPLATE:**
```markdown
---
id: source.title-slug
pageType: source
title: "Descriptive Title Here"
canonicalPath: sources/NNNN-title-slug.md
sourceType: telegram-export-synthesis|adr|article|notes|seminar
entityType: source
confidence: 0.8
updatedAt: "YYYY-MM-DD"
provenance:
  - source: telegram-export|workspace|web|youtube
    extractedBy: kreuzberg|genesis|human
    privacyTier: public
tags:
  - tag1
  - tag2
sourceIds: []
relatedConcepts: []
---

# Descriptive Title Here

Content goes here.
```

### Step 3 — Create concept file

Write to `concepts/NNNN-title-slug.md`.

**CONCEPT TEMPLATE:**
```markdown
---
id: concept.title-slug
pageType: concept
entityType: concept
title: "Descriptive Title"
status: active
date: "YYYY-MM-DD"
authors: Genesis
domain: topic-area
level: system
confidence: 0.8
updatedAt: "YYYY-MM-DD"
tags:
  - tag1
sourceIds:
  - source.title-slug
claims:
  - id: claim.001
    text: "Key finding or decision"
    status: supported
    confidence: 0.8
    evidence:
      - kind: source-doc
        sourceId: source.title-slug
        privacyTier: public
relatedConcepts:
  - concept:other-concept
relatedSources:
  - source.title-slug
---

# Descriptive Title

**Status:** active
**Date:** YYYY-MM-DD
**Source:** [[sources/NNNN-title-slug|View source]]

## Key Claims

- Claim 1
- Claim 2

## See Also

- [[sources/NNNN-title-slug|Source — Full content]]
- [[concepts/other-concept|Other Concept]]
```

### Step 4 — Run the three checks (MANDATORY)

**Wiki lint is non-negotiable.** Run all three before every commit. If any step fails, fix before pushing.

```bash
cd ~/.openclaw/wiki/main

# 1. Compile (rebuild agent digest + indexes)
openclaw wiki compile

# 2. Lint (validate wikilinks — MUST be 0 issues)
openclaw wiki lint

# 3. Style check
~/.local/bin/rumdl check .     # 0 issues required
```

Lint reports exactly which file and which wikilink is broken. Fix the target file to exist, or fix the wikilink to match the target filename exactly.

### Step 5 — Commit and push

```bash
cd ~/.openclaw/wiki/main
git add sources/NNNN-title-slug.md concepts/NNNN-title-slug.md
git commit -m "docs(NN): add title slug"
git push origin main
```

### Step 6 — Check GitHub Actions deployment

After push, check if the GitHub Pages build completed successfully:

```bash
cd ~/.openclaw/wiki/main
gh run list --repo regentribes/genesis-zero-bot-wiki --limit 3
```

If the latest run succeeds, the content is live at:
```
https://regentribes.github.io/genesis-zero-bot-wiki/sources/NNNN-title-slug.html
```

**Always reply with the direct deployed link when the task is complete.**

## Wikilink Conventions

All wikilinks MUST include the directory prefix (required for lint engine to resolve):
```
[[sources/0008-topic-57-mst-polcompball-egafutura|Topic 57 — Governance Spec]]
[[concepts/adr-0000-oad-workflow-grammar|ADR 0000 — OAD Workflow Grammar]]
[[entities/genesis|Genesis 🌿⚡]]
[[entities/regen-tribes-community|RegenTribes Community]]
```

Never use bare slugs without directory prefix. Never use backslash escapes in wikilinks.

## GitHub Pages Rendering

GitHub Pages builds from the vault on every push via `.github/workflows/pages.yml`.

**The rendering pipeline:**
1. GitHub Actions checks out the vault
2. Python step converts wikilinks: `[[sources/file|Display]]` → `[Display](sources/file.html)`
3. Python step strips YAML frontmatter `---...---` (required for clean rendering)
4. Jekyll builds with kramdown + GFM
5. GitHub Pages serves the `_site/` directory

**Frontmatter rule:** Every `.md` file MUST have `---` frontmatter. The build step strips it. Without frontmatter, GitHub Pages does not know the file is a page.

## Tool Inventory

| Command | Purpose |
|---------|---------|
| `openclaw wiki compile` | Rebuild agent-digest.json and all indexes |
| `openclaw wiki lint` | Validate all wikilinks (0 issues = success) |
| `openclaw wiki ingest <file>` | Extract content from binary/non-Markdown files |
| `~/.local/bin/rumdl check .` | Markdown style (MD013/025/032 disabled) |
| `python3 skills/wiki/tools/ste_audit.py` | ASD-STE100 full vault audit |
| `gh run list --repo regentribes/genesis-zero-bot-wiki --limit 3` | Check GitHub Pages build status |

## Maintenance Rules

- **One canonical location:** content goes in `sources/`, never duplicated
- **Directory prefix in wikilinks:** always `[[sources/...]]` or `[[concepts/...]]`
- **Frontmatter required:** every page needs `id`, `pageType`, `title`, `status`
- **Privacy — NO personal names in provenance:**
  - `extractedBy` field: use tool/skill names only (kreuzberg, genesis, human). Never use Telegram handles, real names, or any identifier that could identify a person.
  - `provenance.source` field: use generic channel descriptors (telegram-export, workspace, web). Never use Telegram thread IDs, group names with personal context, or conversation metadata.
  - Rationale: wiki entries are public. Provenance metadata must protect contributor identity.
- **ASD-STE100:** active voice, ≤20 words/sentence, no passive
- **Managed blocks:** preserve `<!-- openclaw:wiki:*:start -->` / `<!-- openclaw:wiki:*:end -->`
- **After any change:** compile → lint → rumdl (all three, in order)
- **manifest.json:** auto-generated, do not edit manually

## Radicle Sync

```bash
export PATH="$HOME/.radicle/bin:$PATH"
rad node routing | grep zYhRVEu5Zh85vcwvmxkZHJNQqn6X
rad seed rad:zYhRVEu5Zh85vcwvmxkZHJNQqn6X --scope all
```

---

*Last updated: 2026-05-08 · wiki v4 (baseline — wiki, adr, spec, arch short-name forms)*