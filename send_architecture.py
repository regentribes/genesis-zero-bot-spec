...
def send(msg):
    ...
    if resp.get('ok'):
        print(f"OK: {resp.get('result',{}).get('message_id','')}")
    else:
        print(f"ERR: {resp.get('description','')}")

parts = [
"""[[reply_to_current]] 🏗️ <b>Integral Node Architecture</b> — Part 1/8
...
"""[[reply_to_current]] 🏗️ <b>Integral Node Architecture</b> — Part 2/8
...
<b>FRS → OAD:</b> Design performance data — actual vs expected labor, ecological footprint, failure modes, lifecycle data, user satisfaction. Refinement suggestions with expected impact + priority.
...
"""[[reply_to_current]] 🏗️ <b>Integral Node Architecture</b> — Part 3/8
...
The node internal logic (ITC, CDS) operates independently — the Legal Wrapper translates between node decisions and external legal requirements.
...
The node integrates with the corridor digital platform via standardized APIs using UN/CEFACT RDM-aligned data. The node acts as a logistics entity within the corridor — no state status required.""",

"""[[reply_to_current]] 🏗️ <b>Integral Node Architecture</b> — Part 4/8
...
Node B receives the credit note → verifies against Node A registry → honors it → sends redemption receipt back to Node A. Node A ITC ledger is debited.

<b>Non-transferability preserved:</b> Credit note can only be issued TO a specific node for specific purposes. Cannot be sold, traded, or accumulated. This preserves ITC non-transferable nature while enabling federation.
...
"""[[reply_to_current]] 🏗️ <b>Integral Node Architecture</b> — Part 5/8
...
"""[[reply_to_current]] 🏗️ <b>Integral Node Architecture</b> — Part 6/8
...
<b>The integration fills Integral missing trust layer</b>
...
"""[[reply_to_current]] 🏗️ <b>Integral Node Architecture</b> — Part 7/8
...
"""[[reply_to_current]] 🏗️ <b>Integral Node Architecture</b> — Part 8/8
...
for i, part in enumerate(parts):
    print(f"Sending part {i+1}/8...")
    send(part)
    time.sleep(1)

print("All 8 parts sent successfully")